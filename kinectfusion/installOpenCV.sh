...
mkdir build
cd build
# changed TBB flag, added CUDA flag, changed DENABLE_PRECOMPILED_HEADERS flag
cmake -DWITH_QT=ON -DWITH_OPENGL=ON -DFORCE_VTK=ON -DWITH_TBB=OFF -DWITH_GDAL=ON -DWITH_XINE=ON -DWITH_CUDA=ON -DBUILD_EXAMPLES=ON -DENABLE_PRECOMPILED_HEADERS=OFF ..
make -j4
make install
ldconfig
